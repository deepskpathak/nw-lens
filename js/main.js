...
function openFolderDialog (folderPathCallback){
    var inputField = document.querySelector('#folderSelector');
    inputField.addEventListener('change', function () {
        var folderPath = this.value;
        folderPathCallback(folderPath);
    });
    inputField.click();
}

function bindSelectFolderClick(folderPathCallback){
    var button = document.querySelector('#select_folder');
    button.addEventListener('click', function(){
        openFolderDialog(folderPathCallback);
    });
}

function hideSelectFolderButton () {
    var heroDiv = document.querySelector('#selector');
    heroDiv.style.display = 'none';
}

function findAllFiles (folderPath, getFilesCallback){
    fs.readdir(folderPath, function (err, files) {
        if(err) {
            return getFilesCallback(err, null);
        }
        getFilesCallback(null, files);
    })
}
...
function findImageFiles (files, folderPath, imageFilesCallback) {
    var imageFiles = [];
